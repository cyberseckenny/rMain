...
import me.kenny.galastic.util.TimeUtils;
...
public abstract class SpecialItem {
    ...
    public SpecialItem(Galastic galastic, String name, List<String> description, int cooldown, int uses, Material material) {
    ...
    public boolean use(Player player, String message) {
        ...
        if (!galastic.getCooldownConfig().isOnCooldown(player, formattedCooldownString)) {
            player.sendMessage(ChatColor.GOLD + message);
            galastic.getCooldownConfig().addCooldown(player, name.toLowerCase().replace(" ", ""), cooldown);
            return true;
        } else {
            int seconds = (int) TimeUnit.MILLISECONDS.toSeconds(galastic.getCooldownConfig().getCooldown(player, formattedCooldownString) - System.currentTimeMillis());
