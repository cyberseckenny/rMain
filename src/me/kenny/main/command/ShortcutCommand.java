...
public class ShortcutCommand implements CommandExecutor {
    ...
    public ShortcutCommand(Main main) {
        this.main = main;
    }
                ...
                if (args[0].equalsIgnoreCase("set")) {
                    if (args.length >= 2) {
                        if (args.length >= 3) {
                            ...
                            main.getShortcutConfig().setShortcut(args[1], commandString);
                        }
                    } else {
                        player.sendMessage(ChatColor.RED + "You must include a command to set this shortcut to!");
                    }
                } else if (args[0].equalsIgnoreCase("remove")) {
                    for (Map.Entry<String, String> entry : main.getShortcutConfig().getShortcuts().entrySet()) {
                        if (entry.getKey().equalsIgnoreCase(args[1])) {
                            if (main.getShortcutConfig().removeShortcut(entry.getKey())) {
                    ...
                    if (main.getShortcutConfig().getShortcuts().size() != 0) {
                    ...
                    for (Map.Entry<String, String> entry : main.getShortcutConfig().getShortcuts().entrySet()) {
                        if (entry.getKey().equalsIgnoreCase(args[0])) {
                            main.getShortcutConfig().doShortcut(player, entry.getKey());
